...
print("[*] Starting Hacker News Auto-Outreach Bot...")

# Unified file to track duplicate leads across PH and HN
...
def send_outreach_email(author_name, project_title, website_url, email_address):
    subject = f"Congrats on your Hacker News launch! 🚀"
    
    body = f"""Hi {author_name},

I saw your project "{project_title}" on Show HN today!

As builders, we know that funding open-source projects or finding sponsorships is a major headache. Most directories charge $99+/mo just to view sponsor lists or contact details.

To help fellow builders, we created CoMatch (https://comatch.org) - a 100% free, open-source matching directory.

🤖 AI Matchmaker: Describe your project, and get matched with B2B sponsors (like Vercel, Supabase, DigitalOcean) or VC funds (like Sequoia, Accel, Seedcamp) instantly.
...
"""
    ...
    if SMTP_PASSWORD == "YOUR_SMTP_PASSWORD" or SMTP_PASSWORD == "":
        print(f"[SIMULATION] Would send email to: {email_address} (Author: {author_name}, Project: {project_title})")
        return True
    ...
    try:
        ...
        server.quit()
        print(f"[+] Successfully sent HN outreach email to: {email_address}")
        return True
...
# Main Execution Loop
try:
    print("[*] Fetching latest Show HN stories...")
    stories_url = "https://hacker-news.firebaseio.com/v0/showstories.json"
    r = requests.get(stories_url, timeout=10)
    story_ids = r.json()
    
    print(f"[+] Found {len(story_ids)} total Show HN stories. Checking top 60 stories...")
    
    new_sent_count = 0
    
    for idx, story_id in enumerate(story_ids[:60], 1):
        try:
            item_url = f"https://hacker-news.firebaseio.com/v0/item/{story_id}.json"
            item_r = requests.get(item_url, timeout=10)
            item = item_r.json()
            if not item:
                continue
            ...
            author = item.get("by", "Developer")
            
            # We only care about stories that link to external projects
            if not website_url:
                continue
                
            # Clean title
            project_title = re.sub(r'(?i)^show hn:\s*', '', title).strip()
            ...
            print(f"\n[*] Processing [{idx}/60] {project_title} by {author} ({domain})...")
            ...
            # Double check domain after redirects
            final_domain = urlparse(final_url).netloc.lower().replace("www.", "")
            if final_domain in sent_domains:
                continue
                
            # Scrape website for emails
            emails = scrape_site_for_emails(final_url)
            if not emails:
                print(f"    [-] No emails found for {final_domain}")
                sent_domains.add(final_domain) # add to skip checking next time
                continue
            ...
            # Send email
            if send_outreach_email(author, project_title, final_url, target_email):
                sent_domains.add(final_domain)
                new_sent_count += 1
            ...
            print(f"[!] Error processing story ID {story_id}: {e_story}")
            
    print(f"\n[+] Hacker News mail run complete! Sent {new_sent_count} new outreach emails.")
except Exception as e:
    print(f"[-] Error during Hacker News outreach: {e}")
